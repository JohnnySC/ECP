...
interface FeatureChain {
    ...
    interface ChekAndHandle : Check, Handle

    class UnknownMessageChain(private val manageResources: ManageResources) : ChekAndHandle {
        ...
        override suspend fun handle(message: String): MessageUI {
            return MessageUI.AiErrorMessageUI(
                errorMessageId,
                "${manageResources.string(R.string.i_dont_understand)} \"$message\""
            )
        }
    }
}